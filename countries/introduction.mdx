...
## Account status (Brazil, Chile, Bolivia)

Accounts have lifecycle status `Operativa`, `Bloqueada`, or `Cerrada`. For **Brazil**, **Chile**, and **Bolivia** direct inbound/outbound APIs and **hosted checkouts** (`POST /api/v1/checkouts`), only **`Operativa`** accounts can receive or send funds. Blocked or closed accounts return HTTP `403` with `code: ACCOUNT_NOT_OPERATIVE`.

Use `GET /api/v1/accounts` or `GET /api/v1/account/{id}/balance` to check status before creating a transaction or checkout.
