...
| **BO** | **Payer name**, **CI/document (`document`)**, and **email** required |
...
- **`payer.name`**, **`payer.document`** (payer CI or document ID), and **`payer.email`** required on create.
- Response may include **`qrCode`** for the payer to scan.
...
Response includes **`checkoutUrl`** — send that URL to the payer. A **`qrCode`** (base64 image) may also be returned on create.
