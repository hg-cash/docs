...
Os payloads têm o mesmo formato independentemente da fila: **`Content-Type: application/json`**. Cada payload inclui **`topic`** e **`eventType`** no nível superior para roteamento. A maioria dos corpos é um objeto JSON plano; **`checkout.completed`** pode incluir também um objeto **`transaction`** aninhado com os dados da movimentação no ledger (mesmos campos de [**Movimentação de conta**](/api-reference/webhooks/account-movement), sem `topic` nem `eventType`).
...
| `CLAIM` | `status_change` | Status da reclamação alterado por um administrador ou por autoconciliação da COELSA |
...
- **Checkouts hospedados** (`event` como `checkout.completed`): [**Eventos de checkout**](/api-reference/webhooks/checkout-completed) — schema **`WebhookCheckoutPayload`**. Em **`checkout.completed`**, quando há uma transação de ledger vinculada, o payload inclui um objeto **`transaction`** aninhado (mesmos campos de [**Movimentação de conta**](/api-reference/webhooks/account-movement), sem `topic` nem `eventType`). Veja **[Checkout hospedado](/pt-BR/developers/checkout)**.
- **Mudanças de status de reclamação** (`topic` `CLAIM`, `eventType` `status_change`): [**Status da reclamação atualizado**](/api-reference/webhooks/claim-status-updated) — schema **`ClaimStatusWebhook`**. Enviado para a sua **URL de webhook padrão** quando um administrador atualiza uma reclamação ou a COELSA a autoconcilia. O corpo corresponde à resposta de **Obter reclamação** (`GET /claims/{id}`, sem `fileUrl`) mais os metadados de roteamento, com um campo **`comment`** opcional. Webhooks **não** são enviados na criação da reclamação nem em atualizações que apenas adicionam um comentário. Veja **[Reclamações](/pt-BR/claims/introduction)**.
