---
title: "Introducción"
description: "Páginas de pago hospedadas para Argentina, Brasil, Chile y Bolivia"
icon: "cart-shopping"
---
...
| **BO** | Pago QR | Transacción entrante del proveedor + webhook; el pagador ve el QR en la página hospedada |
...
Cuando un checkout cambia de estado, HG.Cash puede enviar un webhook **`CHECKOUT`** (`checkout.completed`, `checkout.awaiting_manual_review` o `checkout.rejected`). Configurá **`webhookUrl`** por checkout al crear o la URL por defecto del usuario. En **`checkout.completed`**, cuando hay una transacción de libro vinculada, el webhook incluye un objeto **`transaction`** anidado con los mismos campos de movimiento que un webhook de [**Movimiento de cuenta**](/api-reference/webhooks/account-movement) (sin `topic` ni `eventType`). Ver [Recibir webhooks](/es/developers/receiving-webhooks) para la firma HMAC.
